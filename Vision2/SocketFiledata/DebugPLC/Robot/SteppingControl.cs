...
namespace ErosSocket.DebugPLC.Robot
{
    public partial class SteppingControl : UserControl
    {
        ...
        private EpsenRobot6 EpsenRobot;
        ...
        private void EnbeData(string name)
        {
            try
            {
        ...
        private void 删除点文件ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (System.IO.File.Exists(ProjectINI.ProjectPathRun + "\\点位文件\\" + listBox1.SelectedItem.ToString() + ".xls"))
                    {
                        System.IO.File.Delete(ProjectINI.ProjectPathRun + "\\点位文件\\" + listBox1.SelectedItem.ToString() + ".xls");
                    }
                }
