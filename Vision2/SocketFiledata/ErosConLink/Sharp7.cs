...
namespace Sharp7
{
    ...
    public static class S7
    {
        ...
        /// <returns></returns>
        public static string GetStringAt(byte[] Buffer, int Pos, Encoding encoding)
        {
            int size = (int)Buffer[Pos + 1];
            return encoding.GetString(Buffer, Pos + 2, size);
        }
        ...
        /// <param name="Value"></param>
        public static void SetStringAt(byte[] Buffer, int Pos, int MaxLen, string Value, Encoding encoding)
        {
            int size = encoding.GetBytes(Value).Length;
            Buffer[Pos] = (byte)MaxLen;
            Buffer[Pos + 1] = (byte)size;
            encoding.GetBytes(Value, 0, Value.Length, Buffer, Pos + 2);
        }
    ...
    public class S7Client
    {
        ...
        // S7 Get PLC Status
        private byte[] S7_GET_STAT = {    0x03, 0x00, 0x00, 0x21, 0x02, 0xf0, 0x80, 0x32,  0x07, 0x00, 0x00, 0x2c,
            0x00, 0x00, 0x08, 0x00,  0x08, 0x00, 0x01, 0x12,  0x04, 0x11, 0x44, 0x01,0x00, 0xff, 0x09,
            0x00, 0x04, 0x04, 0x24, 0x00, 0x00    };
        ...
        private int TCPConnect()
        {
            if (_LastError == 0)
                try
                {
                    _LastError = Socket.Connect(IPAddress, _PLCPort + 1);
                }
                catch
                {
        ...
        /// <summary>
        /// 设置属性以与Snap7.net.cs兼容
        /// </summary>
        /// <param name="ParamNumber">参数编号</param>
