    ...
    public partial class SocketConnectForm : Form
    {
        ...
        private string loadDate()
        {
            try
            {
                ...
                if (File.Exists(ErosProjcetDLL.Project.ProjectINI.In.ProjectPathRun +"\\"+DicSocket.Instance.FileName+ "\\XMLSocket.xml"))
                {
                    doc.Load(ErosProjcetDLL.Project.ProjectINI.In.ProjectPathRun + "\\" + DicSocket.Instance.FileName + "\\XMLSocket.xml");
                    //获得根节点
        ...
        {
            if (!StaticCon.WriteSocketXML(dataGridView1, ErosProjcetDLL.Project.ProjectINI.In.ProjectPathRun + "\\" + DicSocket.Instance.FileName + "\\XMLSocket.xml"))
            {
        ...
        private sql.SQLForm sQLForm;

        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            try
            {
                if (sQLForm == null || !sQLForm.Disposing)
                {
                    sQLForm = new sql.SQLForm();
                }
                sQLForm.Show();
            }
            catch (Exception)
        ...
        private SocketDic SocketDic;

        private void 子链接ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (SocketDic == null || !SocketDic.Disposing)
                {
                    SocketDic = new SocketDic();
                }
                SocketDic.Show();
            }
            catch (Exception)
        ...
        private void 打开HMIToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ObjHMI objHMI;
                    string da = dataGridView1.SelectedRows[0].Cells["NameID"].Value.ToString();

                    objHMI = new ObjHMI("", da, StaticCon.SocketClint[da].IP.ToString());
                    objHMI.Show();
                }
            }
            catch (Exception ex)
        ...
        {
            OnManagementForm onManagementForm = new OnManagementForm();
        ...
        {
            //ErosProjcetDLL.ErosDesignSurface.ErosDesign erosDesign = new ErosProjcetDLL.ErosDesignSurface.ErosDesign();
            //erosDesign.Show();
        }
