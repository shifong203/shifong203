       ...
       TypeConverter(typeof(ErosConverter)), ErosConverter.ThisDropDownAttribute("ListName")]
        ...
        public void MarkCiacelbMove(string MarkName1, string axisName = null, HWindID hWindID = null)
        {
            ...
            try
            {
                ...
                Vision.GetRunNameVision(VisionName).GetResultOBj().AddMeassge(Vision.GetRunNameVision(VisionName).GetCalib().GetMatHomString());
                Vision.GetRunNameVision(VisionName).GetResultOBj().AddMeassge(Vision.GetRunNameVision(VisionName).GetCalib().GetRCMatHomString());

                if (hXs.TupleSub(axisX).TupleAbs().TupleMax() < 0.05 && hYs.TupleSub(axisY).TupleAbs().TupleMax() < 0.05)
                {
                    Vision.GetRunNameVision(VisionName).GetResultOBj().AddMeassge("标定成功x误差:" + hXs.TupleSub(axisX).TupleAbs().TupleMax().TupleString("0.03f") + "y误差:" + hYs.TupleSub(axisY).TupleAbs().TupleMax().TupleString("0.003f"));
                }
                else
                {
                    Vision.GetRunNameVision(VisionName).GetResultOBj().AddMeassge("标定失败x误差:" + hXs.TupleSub(axisX).TupleAbs().TupleMax().TupleString("0.03f") + "y误差:" + hYs.TupleSub(axisY).TupleAbs().TupleMax().TupleString("0.003f"));
                }
                Vision.GetRunNameVision(VisionName).GetResultOBj().AddMeassge("机械差X（" + axisX + "/" + hXs + "=" + hXs.TupleSub(axisX) + "）;Y（"
                   + axisY + "/" + hYs + "=" + hYs.TupleSub(axisY) + ")");

                for (int i = 0; i < hRows.Length; i++)
                {
                    Vision.GetRunNameVision(VisionName).GetResultOBj().AddImageMassage(hRows[i], hCols[i], (i + 1));
                }

                if (hWindID != null)
                {
                    hWindID.HalconResult.AddMeassge(Vision.GetRunNameVision(VisionName).GetCalib().GetMatHomString());
                            ...
                            try
                            {
                                ...
                                if (hWindID != null)
                                {
                                    ...
                                    if (asyncRestImage.RunID == XS.Count)
                                    {
                                        ...
                                        FillIamge(hWindID);
                                        hWindID.HalconResult.AddMeassge(watchOut.ElapsedMilliseconds + "ms");
                                    }
                                    hWindID.ShowImage();
                                }
                            }
            ...
            try
            {
                ...
                for (int i = 0; i < XindexC; i++)
                {
                    for (int i2 = 0; i2 < YindexC; i2++)
                    {
                        ...
                        if (hwind != null)
                        {
                            hwind.HalconResult.AddObj(hObject);
                            hwind.HalconResult.AddImageMassage(rowT, colsT, (i2 + 1) + (i * YindexC));
                        }
                    }
                }
            }
