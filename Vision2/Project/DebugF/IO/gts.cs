...
namespace Vision2.Project.DebugF.IO
{
    public class Gts
    {
        ...
        public static string GetErrCode(string code,int errCode)
        {
            string errStr = "";
            string solution = "";
            switch (errCode)
            {
                case -8:
                    errStr = "多线程资源忙";
                    solution = "GT指令在线程里执行超时才返回，有可能是PCI通信异常，导致GT指令无法及时返回";
                    break;
                case -7:
                    errStr = "DSP忙";
                    solution = "调用GT指令后，DSP仍然在处理，不再接收新指令";
                    break;
                case -6:
                    errStr = "打开/关闭设备错误";
                    solution = "打开卡时,没有控制卡、或者控制卡数量超过最大设定值（4张卡）、创建PCI通信区失败关闭卡时，已经没有卡、或者关闭PCI通信区失败";
                    break;
                case -5:
                    errStr = "读取数据块错误";
                    solution = "Windows API 函数向PCI写或读数据出错，例如PCI通信异常，或者没有创建PCI通信就向通信区读写数据";
                    //PCI通信异常的原因很多，需要根据具体问题具体分析。
                    //可以先尝试更换PCI插槽，清除控制卡PCI上金手指的灰尘，实在不行只能寄回工厂对硬件进行检测。
                    break;
                case -4:
                    errStr = "写入数据块错误";
                    solution = "Windows API 函数向PCI写或读数据出错，例如PCI通信异常，或者没有创建PCI通信就向通信区读写数据";
                    ...
                    break;
                case -3:
                    errStr = "读取数据校验和错误";//数据在PCI传输过程中受到干扰，电平发生改变，导致数据被更改，因此是无效数据
                    solution = "数据在PCI传输过程中受到干扰，电平发生改变，导致数据被更改，因此是无效数据";
                    //PCI通信异常的原因很多，需要根据具体问题具体分析。
                    //可以先尝试更换PCI插槽，清除控制卡PCI上金手指的灰尘，实在不行只能寄回工厂对硬件进行检测。
                    break;
                case -2:
                    errStr = "读取数据长度错误";//PCI通信异常，通信区被破坏，数据丢失或包含了杂数据
                    solution = "PCI通信异常，通信区被破坏，数据丢失或包含了杂数据";
                    //PCI通信异常的原因很多，需要根据具体问题具体分析。
                    //可以先尝试更换PCI插槽，清除控制卡PCI上金手指的灰尘，实在不行只能寄回工厂对硬件进行检测。
                    break;
                case 0:
                ...
                case 1:
                    errStr = "错误调用指令";
                    solution = "相关的指令没有调用，该指令的执行条件不满足";
                    break;
                case 7:
                    errStr = "参数错误";
                    solution = "输入指令的参数出错";
                    break;
                case 8:
                    errStr = "DSP固件不支持该指令对应的功能";
                    //solution = "DSP固件不支持该指令对应的功能";
                    break;
                default:
                    break;
            }
            if (errCode!=0)
            {
                ErosProjcetDLL.Project.AlarmText.AddTextNewLine(code+":" +errStr +";"+ solution);
            }
            return errStr;
        }
        [DllImport("gts.dll")]
