...
namespace Vision2.vision.HalconRunFile.RunProgramFile
{
    ...
    public class QRCode : RunProgram
    {
      ...
      ErosConverter.ThisDropDown("", "dark_on_light", "light_on_dark", "any")]
        ...
        private int module_size_mam = 100;
  ...
  ErosConverter.ThisDropDown("", "", "standard_recognition", "enhanced_recognition", "maximum_recognition")]
        ...
        private void threadAnyCodeString(OneResultOBj halcon, HObject hObject)
        {
            number = 0;
        ...
        /// 二维码ID
        /// </summary>
        private List<HTuple> IDs = new List<HTuple>();

        private HTuple resultHandles = new HTuple();

        public void FindS(HObject hObject, OneResultOBj halcon)
        {
            ...
            try
            {
                ...
                if (text.Length != 0)
                {
                    ...
                    if (this.ISShowText)
                    {
                        halcon.AddMeassge("第1次" + text.Length);
                    }
                    AddGreen(hObject1);
                    try
                    {
                ...
                HOperatorSet.AreaCenter(hOQERoi, out area, out HTuple outrow, out HTuple outcol);
                OutRow = outrow;
                OutCol = outcol;
                QRText.Clear();
        ...
        public override bool RunHProgram(OneResultOBj oneResultOBj, out List<OneRObj> oneRObjs, AoiObj aoiObj)
        {
            ...
            try
            {
                ...
                HObject hObject3;
                hObject3 = this.GetEmset(oneResultOBj.Image);
                //SetDataProgram();
                //SetParam(ID);
                if (DiscernType == 2)
                {
                    ...
                    {
                        threadAnyCodeString(oneResultOBj, hObject3);
                    }
                }
                else
                {
                ...
                {
                    oneResultOBj.AddMeassge("识别数量" + number + "/" + this.IDValue);
                }
                if (number > 0)
                {
        ...
        private HObject XLDT = new HObject();
        ...
        /// <param name="hObject"></param>
        /// <returns></returns>
        public HTuple TrainQRCode(HObject image, OneResultOBj halcon, out HObject hObject, List<HObject> iamges = null, List<HObject> objs = null)
        {
            HTuple tupleStr = new HTuple();
            hObject = new HObject();
            HObject hObjectCont = new HObject();
            ...
            try
            {
                ...
                for (int i = 0; i < ThraQR; i++)
                {
                    ...
                    this.Watch.Stop();
                    halcon.AddMeassge("第" + (i + 1) + "\\" + ThraQR + "次数量:" + data.Length + ";时间S:" + this.Watch.ElapsedMilliseconds / 1000);
                    if (data.Length == 0)
                    {
                        break;
                    }
        ...
        public void SrotCode(OneResultOBj halcon)
        {
            try
            {
                this.TrayIDS.Clear();
                ...
                halcon.AddImageMassage(Row2 + 80, cols2, new HTuple(this.TrayIDS.ToArray()));
                Rows = Row2;
                Cols = cols2;
