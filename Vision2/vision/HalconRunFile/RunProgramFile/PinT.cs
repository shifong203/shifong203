...
namespace Vision2.vision.HalconRunFile.RunProgramFile
{
    public class PinT : RunProgram
    {
        ...
        public override bool RunHProgram(HalconRun halcon, OneResultOBj oneResultOBj, int id, string name = null)
        {
            ...
            {
                ISCompound = false;
                halcon.GetHomdeMobel(this.HomName);
                ...
                hObject = halcon.GetModelHaoMatRegion(HomName, hObject);
                //halcon.AddOBJ(hObject);
                //xld = hObject;
                HOperatorSet.ReduceDomain(halcon.Image(), hObject, out FreiAmpImage);
                if (id != 0)
                {
                ...
                try
                {
                    ...
                    CheckPin(halcon, FreiAmpImage, out rows, out columns, id);
                    ...
                    if (EnveType == 0)
                    {
                        ...
                        {
                            halcon.AddMessage("点数量错误" + rows.Length);
                            err++;
                        }
                        if (id < 7)
                        {
                            for (int i = 0; i < rows.Length - 1; i++)
                            {
                                ...
                                if (i % ColumnNumber != ColumnNumber - 1 || RowNumber == 1)
                                {
                                    ...
                                    lengt = halcon.GetCaliConstMM(lengt);
                                    RowMt.Append(lengt);
                                    ...
                                    {
                                        AddGreen(ho_PinSort);
                                        halcon.AddMessageIamge(rowCent, columnCent, new HTuple(lengt).TupleString(".3") + "C");
                                    }
                                }
                                ///Row纵向
                                if (RowNumber > 1 && i < (RowNumber - 1) * ColumnNumber)
                                {
                                    try
                                    {
                                        ...
                                        {
                                           AddGreen(ho_PinSort);
                                            halcon.AddMessageIamge(rowCent, columnCent, new HTuple(lengt).TupleString(".3") + "R");
                                        }
                                    }
                                    catch (Exception ex)   { }
                                }
                    ...
                    {
                        HTuple dintst = halcon.GetCaliConstPx(ColumnMM) / 4;
                        HTuple dintstR = halcon.GetCaliConstPx(RowMM) / 4;
                        for (int i = 0; i < MRows.Length; i++)
                        {
                            ...
                            HOperatorSet.DistancePp(HTuple.TupleGenConst(rows.Length, MRows[i]), HTuple.TupleGenConst(rows.Length, MColumns[i]), rows, columns,
                                out HTuple DINGSE);
                            HTuple DET = halcon.GetCaliConstMM(DINGSE.TupleMin());
                            ...
                            if (DET.TupleMax() > ToleranceMM)
                            {
                                ...
                                halcon.AddOBJ(hObjec.ConcatObj(hObjectxt), ColorResult.red);
                                err++;
                            }
                            else
                            {
                            ...
                            if (id != 0 && id < 6)
                            {
                                halcon.AddMessageIamge(MRows[i], MColumns[i], DET.TupleString("0.02f"));
                            }
                        }
                    }    
                    else
                    {
                        ...
                        if (OKtE ==0)
                        {
                            ...
                            halcon.AddMessageIamge(XaxisRow[0], XaxisCol[0], "X");
                            halcon.AddMessageIamge(YaxisRow[0], YaxisCol[0], "Y");
                            for (int i = 0; i < rows.Length; i++)
                            {
                                ...
                                minlengt= halcon.GetCaliConstMM(minlengt);
                                ...
                                if (iletM.TupleAbs() < RowMM) 
                                {
                                    ...
                                    halcon.AddMessageIamge(rows.TupleSelect(i), columns.TupleSelect(i), "Y" + minlengt);
                                } 
                                else
                                {
                                    halcon.AddMessageIamge(rows.TupleSelect(i), columns.TupleSelect(i),   iletM,ColorResult.red);
                                    err++;
                                ...
                                minlengt1 = halcon.GetCaliConstMM(minlengt1);
                                ...
                                if (iletM.TupleAbs() < ColumnMM) { 
                                    ...
                                    halcon.AddMessageIamge(rows.TupleSelect(i) + 80, columns.TupleSelect(i), "X" + minlengt1);
                                }
                                else
                                {
                                    halcon.AddMessageIamge(rows.TupleSelect(i) + 80, columns.TupleSelect(i), iletM, ColorResult.red);
                                    err++;
                ...
                {
                    halcon.AddNGMessage("间距");
                }
                if (id != 0 && id < 8)
                {
                    return false;
                }
                HObject imagetd = GetEmset(halcon.Image());
        ...
        /// <returns></returns>
        public bool DeFilt(HalconRun halcon, HObject imageT, int runid = 0)
        {
            try
            {
                ...
                if (runid == 8)
                {
                    halcon.AddMessageIamge(row, column, area);
                    halcon.AddOBJ(hObject);
                    return false;
                }
        ...
        /// <param name="runid"></param>
        /// <returns></returns>
        public bool DeFilt2(HalconRun halcon, OneResultOBj oneResultOBj, HObject imageT, int runid = 0)
        {
            try
            {
                if (runid != 0 && runid <= 9)
        ...
        public bool CheckPin(HalconRun halcon, HObject imageT, out HTuple rows, out HTuple columns, int runID = 0)
        {
            bool OKt = false;
            rows = new HTuple();
            columns = new HTuple();
            ...
            try
            {
                ...
                if (runID == 2)
                {
                    ...
                    for (int i = 0; i < rows.Length; i++)
                    {
                        ...
                        halcon.AddMessageIamge(rows[i], columns[i], (i + 1));
                    }
                    return false;
                }
                if (!IsHtoB)
                {
                    ...
                    if (runID == 3)
                    {
                        ...
                        for (int i = 1; i < ho_PinNo.CountObj() + 1; i++)
                        {
                            ...
                            halcon.AddMessageIamge(rows, columns, i);
                        }
                        return false;
                    }
                }
                ...
                if (rows.Length < ColumnNumber * RowNumber)
                {
                    ...
                    halcon.AddMessage("未找到足够的区域3:数量" + rows.Length);
                }
                if (!IsHtoB)
                {
                    ...
                    if (runID >= 4)
                    {
                        halcon.AddOBJ(ho_PinHave);
                        HOperatorSet.AreaCenter(ho_PinNo, out area, out rows, out columns);
                        halcon.AddMessageIamge(rows + 10, columns, "面积" + area);
                        ...
                        halcon.AddMessageIamge(rows + 40, columns, "内半径" + area);
                        if (runID==4)
                        {
                            return false;
                        }
                ...
                {
                    halcon.AddOBJ(ho_PinNo, ColorResult.blue);
                    halcon.AddMessage("未找到足够的区域6：数量" + d);
                }
                HObject hObject = new HObject();
                    ...
                    else
                    {
                        ...
                        if (!SelsMax)
                        {
                            ...
                            if (runID != 0)
                            {
                                ...
                                halcon.AddMessageIamge(rows + 100, columns, (i + 1), ColorResult.red);
                        ...
                        if (!SelsMax)
                        {
                            ...
                            if (mean2.Length > 1)
                            {
                                ...
                                {
                                    HOperatorSet.AreaCenter(hObject, out area, out rows, out columns);
                                    halcon.AddMessageIamge(rows, columns, mean2, ColorResult.red);
