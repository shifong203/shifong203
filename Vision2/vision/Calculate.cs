...
namespace NokidaE.vision
{
    ...
    public class Calculate : RunProgram
    {
        ...
        protected override bool RunHProgram(HalconRun halcon,int runid=0)
        {
           
            try
            {
                HOperatorSet.EdgesImage(halcon.Image(), out HObject hObject1, out HObject hObject2, Filter,Alpha, NMS, Low, High);
                HOperatorSet.Threshold(hObject1, out hObject1, thresholdLow, thresholdHigh);
                HOperatorSet.Connection(hObject1, out hObject1);
                HOperatorSet.SelectShape(hObject1, out xld, "height", "and", 30, 999999);
                return true;
            }
            catch (Exception ex)
            {
