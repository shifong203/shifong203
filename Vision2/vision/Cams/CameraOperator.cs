...
namespace NokidaE.vision.Cams
{
    ...
    internal class CameraOperator
    {
        ...
        /// </summary>
        /// <param name="strKey">参数键值，具体键值名称参考HikCameraNode.xls文档</param>
        /// <param name="nValue">设置参数值，具体取值范围参考HikCameraNode.xls文档</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="strKey">参数键值，具体键值名称参考HikCameraNode.xls文档</param>
        /// <param name="fValue">设置参数值，具体取值范围参考HikCameraNode.xls文档</param>
        /// <returns> 成功：0；错误：-1</returns>
        ...
        public int GetEnumValue(string strKey, ref MyCamera.MVCC_ENUMVALUE pstValue)
        {
            int nRet = m_pCSI.MV_CC_GetEnumValue_NET(strKey, ref pstValue);
            if (MyCamera.MV_OK != nRet)
            {
                return CO_FAIL;
            }
        ...
        ///  保存图片
        /// </summary>
        /// <param name="pSaveParam">保存图片配置参数结构体</param>
        /// <returns>  成功：0；错误：-1</returns>
        public int SaveImage(ref MyCamera.MV_SAVE_IMAGE_PARAM_EX pSaveParam)
        {
            return m_pCSI.MV_CC_SaveImageEx_NET(ref pSaveParam);
        }

        public int ForceIp(IntPtr pDeviceInfo, UInt32 nIp, UInt32 nSubMask, UInt32 nDefaultGateWay)
        {
            int nRet = -1;

            do
            {
                nRet = CreateDevice(pDeviceInfo);
                //if (nRet != MyCamera.MV_OK)
                //{
                //    nRet = CO_FAIL;
                //    break;
                //}
                nRet = m_pCSI.MV_GIGE_ForceIpEx_NET(nIp, nSubMask, nDefaultGateWay);
                if (nRet != MyCamera.MV_OK)
                {
                    nRet = CO_FAIL;
                    break;
        ...
        /// <returns></returns>
        public int CreateDevice(IntPtr pDeviceInfo)
        {
            ...
            int nRet = m_pCSI.MV_CC_CreateDevice_NET(ref device);
            if (MyCamera.MV_OK != nRet)
            {
                return CO_FAIL;
            }
