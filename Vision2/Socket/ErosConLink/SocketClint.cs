...
namespace ErosSocket.ErosConLink
{
    ...
    public class SocketClint : ProjectC, IErosLinkNet, IReSendSocke, ProjectNodet.IClickNodeProject
    {
        ...
        /// 接受事件
        /// </summary>
        void ReadMesage(string mesage, Socket socketR)
        {
        ...
        public virtual string AlwaysReceive(int de)
        {
            ...
            try
            {
                ...
                Insocket.ReceiveTimeout = de;
                ReciveBufferLenth = Insocket.Receive(Recivebuffer);
                 dataStr = GetEncoding().GetString(Recivebuffer.Skip(0).Take(ReciveBufferLenth).ToArray());
                if (FTU == "CRLF")
                {
                    dataStr = GetEncoding().GetString(Recivebuffer.Skip(0).Take(ReciveBufferLenth).ToArray()).Replace("\r\n", "");
                }
                else if (FTU == "CR")
                {
                    dataStr = GetEncoding().GetString(Recivebuffer.Skip(0).Take(ReciveBufferLenth).ToArray()).Replace("\n", "");
                ...
                {
                    dataStr = GetEncoding().GetString(Recivebuffer.Skip(0).Take(ReciveBufferLenth).ToArray()).Replace("\r", "");
                }
                return dataStr;
            }
            catch (Exception re)  {   }
        ...
        void CallBcakMethod(IAsyncResult asyncResult)
        {
            try
            {
        ...
        {
            return new NewSocketForm(this) { TopLevel = false };
        }
        Values values;
        NewSocketForm newSocketForm;
        /// <summary>
        /// 
        /// </summary>
